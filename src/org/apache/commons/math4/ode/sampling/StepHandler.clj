(ns org.apache.commons.math4.ode.sampling.StepHandler
...
(defn init
  ...
  ([^StepHandler this ^Double t-0 y-0 ^Double t]
    (-> this (.init t-0 y-0 t))))

(defn handle-step
  "Handle the last accepted step

  interpolator - interpolator for the last accepted step. For efficiency purposes, the various integrators reuse the same object on each call, so if the instance wants to keep it across all calls (for example to provide at the end of the integration a continuous model valid throughout the integration range, as the ContinuousOutputModel class does), it should build a local copy using the clone method of the interpolator and store this copy. Keeping only a reference to the interpolator and reusing it will result in unpredictable behavior (potentially crashing the application). - `org.apache.commons.math4.ode.sampling.StepInterpolator`
  ...
  ([^StepHandler this ^org.apache.commons.math4.ode.sampling.StepInterpolator interpolator ^Boolean is-last]
